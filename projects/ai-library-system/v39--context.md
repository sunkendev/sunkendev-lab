...
- Context-awareness thresholds found stale (2026-07-02): Tier 1/2 at 130K/155K encode absolutes derived from the v18 event's 200K window; current models (Fable 5, Opus 4.8, Sonnet 4.6/5) carry 1M windows in Claude Code, so the ~138K Tier 1 alarm this session fired at ~14% of the real window — the durable v18 finding is the ratio (warn ~65%, act ~77.5%), not the numbers
- Design rule: benefit from window headroom, encode only ratios, never hardcode the window — window size is a vendor runtime property the library must not bake assumptions on
- 1M windows recompute the growth wall from ~v132 to past v600 — THREAD.md archiving drops from urgent to dormant; caveat recorded that the wall analysis measured fit, not long-context attention quality
- Industry pairing of long windows with compaction, memory files, and structured note-taking validates the library's design precisely: raw transcript is not memory; the substitute got five times better and did not displace the checkpoint/RESUME model
- Proposed, pending design: a tracked model/context-window config table the skill reads (model / environment / window / observed compression boundary; conservative 200K default), and a context-probe test skill that finds the native compression boundary empirically per environment (harness compaction can fire well before the nominal window — v18 was compression at 165K on 200K)
- Ruling: no permission allowlist — a committed .claude/settings.json pre-authorising skill commands was rejected; the tool-layer human approval gate stays, at the accepted cost of timed-out prompts when the approver is away
- Operating protocol for harness-level git-step failures (approval stream dropped): inspect git log/git status to confirm whether the command ran before retrying — prevents double commits; used successfully three times on 2026-07-02
- Ruling: checkpoints are produced 100% natively through the ritual — the AI emits all four blocks as direct output and assembles temp/v[NN]-checkpoint.txt itself; no ad-hoc scripts or tools may generate or transform block content; ephemeral helpers in session temp space are tolerated only for incidental non-ritual mechanics and never enter the library
- v38's input-assembly deviation (a scratchpad script relabelled the artifact and spliced the context) was independently verified harmless: artifact differs from v37 in exactly the label and two approved diagram lines, deployed MASTER-PROMPT.md byte-identical, all v37 DECISIONS/RULED OUT lines an exact ordered prefix of v38's; v38 stands, no redo — the ruling applies forward
- Enforcement added to the kernel at v39: behaviour rule 'follow documented procedures by the letter — propose improvements, never adopt unilaterally; an unapproved deviation is an error even when the output is correct'; behaviour rule 'narrate method, not only outcome'; checkpoint-ritual clause 'all four blocks are produced as your direct output — never generate or transform block content with ad-hoc scripts or tools'; matching persona.md BEHAVIOUR line
- Enforcement rationale recorded: content-identical outputs are mechanically undetectable, so process law plus mandatory method-narration are the enforceable layers; building DELTA properly remains the incentive-removing fix
- The v37 bootstrap session's NOTE (git_sync.py fresh-repo edge case — main bootstrapped manually at 1dbc2a8) was found stranded on the dead default branch claude/new-session-iie1if and re-entered via add_note_thread.py with provenance; cherry-pick rejected as unnecessary git surgery
- Orphaned-NOTE gap identified: NOTE's push-only deferral assumes the next sync happens on the same branch; cloud sessions start fresh branches, so a NOTE on a dead session branch is orphaned, not deferred — mitigation open
- GitHub default branch was still claude/new-session-iie1if (first ref pushed to the born-empty repo); user flipped the default to main on 2026-07-02 — the repo landing view now shows current state
- BOOTSTRAP.md, cited by v37's records as the import procedure, was never committed to the repo — a files-are-truth gap; recovery or reconstruction open
RULED OUT:
...
- Committed permission allowlist (.claude/settings.json) for skill commands — rejected by the user; keeps every privileged command behind live human approval despite mobile timeout misses
- Ad-hoc or ephemeral scripts generating/transforming checkpoint block content — banned by ruling; blocks are direct AI output, deterministic help only via deployed tested library scripts adopted through the ritual
- Cherry-picking the stranded bootstrap NOTE onto the session branch — rejected as git surgery risking manual THREAD.md conflict resolution; re-entry via add_note_thread.py with provenance chosen instead
- Redoing v38 natively — rejected; content verified byte-equivalent, and deleting published history would violate more rules than the deviation it would repair
OPEN:
...
- Model/context-window config table (model / environment / window / observed compression boundary) for the skill — proposed, not yet designed
- Context-probe test skill to measure the native compression boundary per environment — proposed, not yet designed
- git_sync.py fresh-repo fix: bootstrap main when origin lacks it; correct the misleading push-failed classification
- Orphaned-NOTE mitigation: session-end sync discipline or RESUME check for unmerged session branches
- BOOTSTRAP.md recovery or reconstruction into projects/ai-library-system/docs/
- RESUME read-set expansion (include the latest artifact now that budget allows) — opportunity, not yet decided
- THREAD.md growth mitigation dormant (wall past v600 at 1M windows) — constrained to verbatim archiving if ever needed
...
- DELTA design for checkpoint.py — now doubly motivated as the incentive-removing enforcement fix
- Whether to fold logs/git-sync.log into logs/checkpoint-runs.log — currently separate
- check_map_integrity false positives on slash-containing prose — scope the regex to markdown link targets
- Eval / quality gate before a new MASTER-PROMPT.md is deployed
- 'Streaming human-LLM interaction as a resumable memory layer' idea — paused; adjacent to Layer 4, not re-raised
- MAP.md references gitignored logs/git-sync.log — keep, drop, or track
- Stale session branch cleanup via GitHub UI: claude/new-session-iie1if deletable now; claude/resume-3hqfn4 only after this checkpoint's fold to main is confirmed
STATE:
- v39 adds the enforcement rules to the kernel (two behaviour rules and the ritual direct-output clause) plus the matching persona.md line; produced 100% natively end to end, as mandated by the v38 post-mortem
- MASTER-PROMPT.md v39 deployed verbatim via the .deploy marker; six post-v38 NOTEs fold into main with this checkpoint's sync
- GitHub default branch now main (user-flipped 2026-07-02); origin/main was current at every sync this session
- Session estimate ~170K by the stale 200K-window model (Tier 2 label per current skill text) — roughly 17% of the actual 1M window; threshold modernisation pending the window config table
