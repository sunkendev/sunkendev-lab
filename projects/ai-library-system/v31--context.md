...
STATE:
- v31: ARCHITECTURE.tex and USER-GUIDE.tex rewritten from scratch (v2.0 / v1.0), closing the long-open "ARCHITECTURE.tex discrepancies" and "USER-GUIDE.tex not yet rewritten" items; originals archived to docs/ as historical record
- Three successive critique re-reads against the new docs found and fixed real drift, including one actual code bug (checkpoint.py's hardcoded .md artifact extension, now generalised via resolve_artifact_extension()/find_artifact_path()) and five separate instances of overstating what RESUME reads (it never reads the artifact file — only persona.md, THREAD.md, latest context.md, latest instructions.md)
- Four commits land this session on top of v30: 7c49c80 (doc rewrite), 31f11cd (first rigor-pass fixes + checkpoint.py extension bug fix), cd7964c (second rigor-pass fixes), 5120f8d (third rigor-pass fixes)
- The .mcp-config.json example OPEN item from v30 (listed eliminated folders) is resolved as a side effect of the rewrite — the new ARCHITECTURE.tex example uses only current folder names
- The "checkpoint.py extension handling if non-.md artifact ever needed" OPEN item from v30 is resolved — the extension bug is fixed and verified end to end
- MASTER-PROMPT.md unchanged this session; artifact block is NO CHANGE, relabelled from v30
- Remaining OPEN items are unchanged from v30 except for the four closed above and one new item surfaced by this session's work (no automated test coverage for checkpoint.py)
