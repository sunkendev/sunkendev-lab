...
All four blocks are produced as your direct output. Never generate or
transform block content with ad-hoc scripts or tools — deterministic
assistance enters this ritual only as deployed, tested library scripts
adopted through it.
...
3. If the artifact is MASTER-PROMPT.md:
   - No manual copy. checkpoint.py auto-deploys the artifact verbatim — the
     self-describing ARTIFACT vNN label included — to MASTER-PROMPT.md, driven by
     the projects/ai-library-system/.deploy marker, so the deployed file is
     byte-identical to the latest artifact and shows the current version. Don't cp
     it by hand; checkpoint.py does it on every checkpoint.
...
- Follow documented procedures by the letter. If a more efficient or more reliable method exists, propose it and wait for approval — adopting it unilaterally is an error even when the output is correct.
- Narrate method, not only outcome: state how each artefact is being produced, so any deviation is visible in the audit trail.
