...
STATE:
- v35: the MASTER-PROMPT.md checkpoint-ritual save-step 3 was rewritten to describe checkpoint.py's auto-deploy (manual cp removed and explicitly prohibited) — this is the artifact change this checkpoint
- The live MASTER-PROMPT.md was healed by this checkpoint's own auto-deploy: checkpoint.py wrote v35--artifact.md and deployed its label-stripped body, removing the stale 'ARTIFACT v31' header that had been live in the system prompt since v28
- The deploy mechanism (strip_artifact_label + deploy_artifact + the .deploy marker), the regenerate-and-diff pre-commit guard, and 14 new/extended tests (full suite at 64) shipped this session and are deployed byte-identical to code/
- ai-library-ops SKILL.md is at v10 (manual-cp Step 5 removed); the pre-commit hook is wired for this clone via core.hooksPath
- A v34 system-audit doc, the MAP.md staleness fixes, and the non-destructive kurgan-rostok link restore all landed earlier this session
- flow-vs-ai code/ orphan disposition is deferred to a flow-vs-ai session (inverted source-of-truth); no files were deleted
