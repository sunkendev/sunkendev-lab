...
- test_git_sync.py not yet built — git_sync.py itself is extracted, live-verified across four scenarios, and already in production use, but has no automated regression test the way checkpoint.py now does
- Whether to fold the new logs/git-sync.log into logs/checkpoint-runs.log, or keep them as two separate files — currently separate, not revisited since creation
- "Streaming human-LLM interaction as a resumable memory layer" idea — explicitly paused by the user; revisit only after the library is otherwise finished, and only if the user re-raises it
STATE:
- v33: MASTER-PROMPT.md unchanged this session; artifact block is NO CHANGE, relabelled from v32
- This session closed out the two test-coverage/observability gaps surfaced at v32: (1) test_checkpoint.py was built, verified (25/25 passing in both deployed locations), committed, and synced to main; (2) the SKILL.md git-sync logic was extracted into git_sync.py, live-verified against four scenarios in isolated throwaway repos (catching and fixing a real diverged-main misclassification bug in the process), wired into SKILL.md v9, committed, and then used on itself for its first real production sync
- Before building either, the session worked through several scoping decisions: confirmed no separate repo for tests/scripts (single-repo, no-CI, no-manifest, stdlib-only constraint adopted); critiqued the signal-lost prose-voice "gap" as a test-authoring artifact rather than a schema flaw, downgrading the floated style-anchor field to ruled-out; researched and rejected JSON Lines logging in favour of staying consistent with the existing plain-text log convention; researched and confirmed Anthropic's own SKILL.md authoring guidance, which validated extracting deterministic git-sync logic out of prose into a script
- A user-floated idea (using an observability/agent platform, then alternatively a raw interaction-stream, as a future memory layer) was discussed, critiqued on vendor-lock-in and unbounded-growth grounds, and explicitly paused by the user to revisit only after the library is otherwise finished
- NOTE's push-only (vs. full main-sync) behavior was re-examined via explicit pros/cons and reconfirmed as-is
- test_git_sync.py was deliberately not built this session — the script extraction and its live verification were treated as the complete unit of work, per explicit user instruction to scope test-building as a separate, later step
