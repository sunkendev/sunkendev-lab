...
STATE:
- v30: SHA-256 manifest/tamper-detection mechanism removed from checkpoint.py end to end, closing the OPEN items "SHA-256 manifest -> native git replacement" and the v29 cross-project MAP.md/THREAD.md tamper blind spot — both resolved by removal rather than by fixing the comparison logic
- Code change (sha256_of_file, manifest_path, write_manifest, load_manifest, Check 4 tamper detection, pre_write_hashes plumbing removed from both code/checkpoint.py and projects/ai-library-system/code/checkpoint.py; .gitignore's manifest exception dropped) committed and pushed as ba0292c ahead of this checkpoint
- This checkpoint's artifact carries the corresponding MASTER-PROMPT.md wording fix: the temp/ folder convention no longer references v[NN]-[slug]-manifest.json
- Root MASTER-PROMPT.md was found stale at the "ARTIFACT v28" label despite v29 having been a NO CHANGE relabel of the project artifact file — this checkpoint's root copy step corrects that drift as a side effect
- 19 historical manifest JSON files remain on disk and in git as archived audit records; no historical data was deleted
- Remaining OPEN items unchanged from v29 except for the two closed above; next priority is unspecified among ARCHITECTURE.tex/USER-GUIDE.tex, temp/ cleanup in flow-vs-ai, or another OPEN item
