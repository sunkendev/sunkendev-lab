...
- No automated test suite for checkpoint.py — the .tex-extension fix this session was verified manually with a synthetic test project, not via a repeatable test; future behavioural changes to checkpoint.py have no regression check
- No automated test coverage for ai-library-ops SKILL.md's git-sync logic either — this session's NOTE push/ff-main gap, and the subsequent push-only revert, were both found and verified only by live manual execution; a real bug shipped in documented-but-unexercised procedure text and was caught only because the user asked a follow-up question, not by any check
- Whether an optional short verbatim style-anchor excerpt should be added to context.md (separate from the full artifact, which stays unread by RESUME) to address the confirmed prose-voice-continuity gap surfaced by the signal-lost rigor test — proposed as a future enhancement, not implemented, not yet requested
STATE:
- v32: MASTER-PROMPT.md unchanged this session; artifact block is NO CHANGE, relabelled from v31
- This session's work was entirely operational/process, not artifact content: (1) the signal-lost rigor test (3 real content-writing checkpoint rounds plus a comprehension-only round, run via genuinely fresh memory-less subagents) validated that RESUME/CHECKPOINT works for sustained real content, not just mechanics, and produced a findings report now permanently in the library; (2) a recurring git-signing "Unverified" warning was fully diagnosed and confirmed to be deliberate, documented Anthropic cloud-sandbox security design, unrelated to the library, and recorded via NOTE; (3) the ai-library-ops skill's push/fast-forward-main sync step — present in CHECKPOINT and COMMIT — was found to be both bypassable (the agent called the underlying scripts directly via Bash, skipping it) and, separately, entirely absent from the NOTE operation's documented procedure
- Both SKILL.md gaps were fixed this session: a warning against calling checkpoint.py/add_note_thread.py directly was added, and a push/ff-main step was added to NOTE — then critiqued against the artifact's own design spirit (NOTE is deliberately the lowest-ceremony operation; CHECKPOINT-weight git plumbing on every note contradicts that and increases merge-conflict exposure) and reverted to push-only, deferring the main fast-forward to the next CHECKPOINT/COMMIT/session-end, consistent with MASTER-PROMPT.md's own description of NOTE
- The push-only revert was verified by live execution (real NOTE commit, real push, real comparison of branch HEAD vs. origin/main HEAD before and after), not just by reading the edited prose — surfaced as a new OPEN item: this skill's git-sync logic has no automated test coverage at all, and this session's actual bug (NOTE silently missing its sync step since v8) was caught only by a user follow-up question, not by any check
- Several NOTEs accumulated in THREAD.md since v31 (branching convention adopted, best-effort branch auto-delete rejected after live testing, the signing-gap finding, the two SKILL.md fixes, and the push-only verification) — all folded into this checkpoint's DECISIONS/RULED OUT above
- main was fast-forwarded to match the session branch multiple times this session, each verified by comparing commit hashes directly rather than assumed from command success alone
