...
OPEN:
- Verify a repo-local .claude/skills skill autoloads and auto-triggers in a cloud session (first cloud session is the spike)
- Verify cloud-session plan-tier eligibility for private repos
- Verify CLAUDE.md @-import behaviour across Claude Code versions
- GitHub private repo: created and populated via manual upload (in progress)
...
STATE:
- v28: library forked to a private GitHub repo for iOS / cloud access; Cowork copy frozen at v28 as baseline
- Mobile-ready files prepared in Cowork for manual upload: CLAUDE.md, .gitignore exception, skill at .claude/skills/ai-library-ops/, code/githooks/pre-commit, and this v28 checkpoint input
- MASTER-PROMPT.md v28: structure diagram adds CLAUDE.md / .claude/skills/ / code/githooks/; folder conventions and the MASTER-PROMPT special case updated for CLAUDE.md Layer 2
- Next: manual upload to GitHub, then the first iOS cloud session runs the v28 checkpoint (doubles as the toolchain spike), then evolve v29 on GitHub
