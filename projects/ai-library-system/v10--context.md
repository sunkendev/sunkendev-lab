...
- ARCHITECTURE.tex discrepancies identified but not yet corrected — deferred
...
- checkpoint.py artifact extension hardcoded to .md — correct for all current projects
...
- MASTER-PROMPT.md updated with tilde fence rule at commit 3a6b816
- CHECKPOINT FILE block added as step 5 of checkpoint ritual — contains all four blocks with fences for direct use as checkpoint.py input
- checkpoint.py input file must be saved outside the library folder — pre-commit hook treats everything inside as a library file
- Version guard added to checkpoint ritual — THREAD.md must be present before checkpoint proceeds
- related: fields in standalone docs use bare filenames not relative paths — pre-commit hook should warn
...
- Saving checkpoint input file inside library folder — pre-commit hook blocks it
...
- checkpoint.py live test not yet completed successfully

STATE:
- MASTER-PROMPT.md at v10 — CHECKPOINT FILE block added, version guard added
- checkpoint.py deployed and hardened against fence collision
- Three NOTEs committed since v09: tilde fix, CHECKPOINT FILE addition, version guard
- checkpoint.py live test attempted but failed — input file format was wrong
- Input file format now understood: save CHECKPOINT FILE block contents outside library, feed to script
- Ready for first successful checkpoint.py live test using this checkpoint output
