...
\hrule height 0.4pt
\vspace{28pt}
{\small Version 2.1 \quad --- \quad \today}
\vfill
...
4 & Synthesis & Cross-project reasoning-pattern synthesis: an external,
                read-only system that reads the whole library to learn
                how the user reasons, producing a versioned,
                evidence-derived meta-persona. Not yet implemented. \\
...
This is version 2.1 of this document. Version 2.0 (2026-06-28)
superseded the original written 2026-04-11 alongside the project's
founding documents; the original is archived at
\texttt{projects/ai-library-system/docs/2026-04-11--architecture-v1-original--claude.tex}.
Version 2.1 (2026-07-02) redefines Layer 4.
...
\begin{itemize}
  ...
  \item Synthesis of reasoning patterns across projects --- that is
...
\begin{enumerate}
  ...
  \item Run it again with \texttt{--write}. If the project declares
        deploy targets (via its \texttt{.deploy} marker ---
        \texttt{ai-library-system} declares \texttt{MASTER-PROMPT.md}),
...
\texttt{checkpoint.py} dry-run then \texttt{--write} (which
auto-deploys \texttt{MASTER-PROMPT.md} verbatim when it is the
artifact), commits, then pushes and fast-forwards \texttt{main}. \\
...
\begin{itemize}
  \item Cross-project reasoning-pattern synthesis --- that is Layer 4
...
% ---------------------------------------------------------------
\section{Layer 4 --- Synthesis}

\begin{layerbox4}
\textbf{Layer 4: Synthesis} \quad Requires a separate repository and an AI with native file-reading tools.\\
...
library as a corpus --- every project's \texttt{THREAD.md} arc, the
...
learned about them and correct it. Where \texttt{persona.md} is the
...
\subsection{Why the Original Retrieval Design Was Retired}
...
\texttt{MAP.md}, one schema everywhere) make it maximally legible to
...
\subsection{Architecture: External and Read-Only}
...
\begin{itemize}
  \item \textbf{The library stays a pure corpus.} A synthesis engine
  ...
  \item \textbf{The record cannot be contaminated.} Read-only access
  ...
  \item \textbf{The vendor boundary lands cleanly.} The library-facing
...
\subsection{The Corpus and Its Limits}

What the library offers as signal, in roughly descending order of
value:

\begin{itemize}
  \item \textbf{RULED OUT registers} --- rejected options with recorded
        rationale: preference pairs, the strongest available signal of
        where the user's boundaries lie
  \item \textbf{Reversals} --- decisions overturned later, with both
        states preserved by the append-only rule: direct evidence of
        what kinds of arguments change the user's mind
  \item \textbf{DECISIONS registers} --- the settled positions themselves
  \item \textbf{THREAD.md arcs} --- the temporal order in which positions
        were reached, held, and revised
  \item \textbf{persona.md} --- the self-declared baseline the synthesis
...
\begin{itemize}
  \item \textbf{The corpus is AI-mediated.} Checkpoint blocks are
  ...
  \item \textbf{The synthesis needs a falsification path.} Without an
  ...
  \item \textbf{It is a decision-policy model, not a mind.} The corpus
...
\begin{itemize}
  \item \textbf{Where the output lives.} Either the synthesis stays in
  ...
  \item \textbf{Coupling to the library's growth mitigation.} Any
        future archiving or compression of \texttt{THREAD.md} decides
...
\begin{itemize}
  \item Writes to the library --- never, under any design
  \item Retrieval infrastructure --- native agentic file search
        already covers finding relevant prior work
  \item Content-level cross-project synthesis (connections and
        contradictions between project subject matter) --- that is
        Layer 5's synthesis agent; Layer 4 synthesises the reasoning,
        not the content
  \item Automatic or scheduled synthesis runs --- scheduling is Layer 5
  \item Recreation of the user's cognition --- the corpus supports a
...
\begin{itemize}
  ...
  \item \textbf{Cross-project content synthesis:} An agent that reads
...
\node[box, fill=l4color!15, draw=l4color, text=l4color!80!black, below=0.3cm of l5]
  (l4) {Layer 4 --- Synthesis (external, read-only)};
...
\begin{enumerate}
  ...
  \item \textbf{Layer 4:} Create the external synthesis repository and
        ...
        \texttt{persona.md} and held-out decisions before trusting it.
...
\begin{center}
{\small\texttt{AI-Library / ARCHITECTURE.tex} \quad Version 2.1 \quad \today}\\[6pt]
