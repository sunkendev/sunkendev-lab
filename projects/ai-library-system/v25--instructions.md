...
BACKGROUND: This session addressed four of the top-5 findings from a Claude Code architecture and code review: (1) SHA-256 manifests committed to git via .gitignore exception — tamper detection audit chain is now durable; (2) code/ added to the library structure diagram in MASTER-PROMPT.md; (3) pre-commit artifact label validation added — ARTIFACT v[NN] presence and version match now enforced; (4) docs/inbox MAP.md blocking corrected from err() to warn() — aligns with the RULED OUT entry. Finding #5 (DELTA design for checkpoint.py) remains open.
ARTIFACT STATE: MASTER-PROMPT.md v25 — code/ added to library structure diagram. Approximately 111 lines.
KEY DECISIONS:
- SHA-256 manifests now committed to git — .gitignore uses temp/* + !temp/*-manifest.json exception
- Library structure diagram updated to include code/ at library root
- pre-commit check_artifact_wrapper() validates label line and version match
- pre-commit check_map_coverage() warns (not errors) for all missing MAP.md entries
OPEN QUESTIONS:
